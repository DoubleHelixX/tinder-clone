...
S.MatchedTimestamp = styled.p`
  ...
  font-size: 14px;
`;
S.MessagesContainer = styled.div``;
        ...
        <S.MessagesContainer>
          {messages.map((message, idx) =>
            message.firstName || message.lastName ? (
              <S.MessageContainer
                key={`${message.firstName}-container--${idx}`}
              >
                <S.MessageImg
                  alt={`${message.firstName} ${message.lastName}`}
                  src={message.url}
                  key={`${message.firstName}-img--${idx}`}
                >
                  {`${message.firstName.charAt(0)} ${message.lastName.charAt(
                    0
                  )}`}
                </S.MessageImg>

                <S.Message key={`${message.firstName}-message--${idx}`}>
                  {message.message}
                </S.Message>
              </S.MessageContainer>
            ) : (
              <S.MessageContainer key={`${message.lastName}-container--${idx}`}>
                <S.MessageUser key={`${message.lastName}-message--${idx}`}>
                  {message.message}
                </S.MessageUser>
              </S.MessageContainer>
            )
